...
public interface TodolistApi {
    ...
    ResponseEntity<ArrayTodolist> todolistGet(@Parameter(in = ParameterIn.QUERY, description = "Is include done todolist" ,schema=@Schema( defaultValue="false")) @Valid @RequestParam(value = "include_done", required = false, defaultValue="false") Boolean includeDone, @Size(min=1,max=100) @Parameter(in = ParameterIn.QUERY, description = "Filter todolist by name" ,schema=@Schema()) @Valid @RequestParam(value = "name", required = false) String name);


    @Operation(summary = "Create new todolist", description = "Create new todolist to database", security = {
        @SecurityRequirement(name = "TodolistAuth")    }, tags={ "Todolist" })
    @ApiResponses(value = { 
